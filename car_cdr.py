class elisp:
    def __init__(self):
        self.destroy_nest = lambda n: self.destroy_nest(n[0]) if self.isiterable(n) and len(n) == 1 else n
    ...
    def d(self,n,*a):
        ...
        if a: return list(a)
        if not self.isiterable(n): return n
        if len(n) == 1: return n[0]
        if self.isiterable(n[1:]) and len(n[1:]) == 1: return n[1:][0]
        else: return list(n[1:])

    def a(self,n,*a):
        n = self.destroy_nest(n)
        if a: return n
        if self.isiterable(n): return n[0]
        else: return n
